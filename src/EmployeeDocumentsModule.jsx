...
export function EmployeeDocumentsPanel({ person, access, legacyFiles = [], compact = false }) {
  ...
  async function documentAction(document, action) {
    ...
    try {
      ...
      if (action === "version") {
        if (!window.confirm(`Create version ${document.version + 1} of ${document.title}? The current version will remain available as superseded.`)) return;
        await api.createEmployeeDocumentVersion(document.id);
      }
      if (action === "archive") {
      ...
      setMessage({ tone: "good", text: action === "send" ? "Document sent to the employee." : action === "generate" ? "PDF generated." : action === "version" ? "New draft version created. The previous version remains in the timeline." : action === "archive" ? "Document archived." : "Secure document link opened." });
      await load(false);
    } catch (error) {
