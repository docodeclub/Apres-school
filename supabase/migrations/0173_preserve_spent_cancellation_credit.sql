-- A later provider reconciliation can replace an invoice's textual
-- "cancelled_credit" state with "bank_confirmed". Credit is still owed when
-- the amount paid exceeds the amended invoice total, so derive the target
-- balance from the amounts rather than transient status labels.
...
-- Reconcile existing invoices with the amount-based rule. This restores any
-- valid cancellation or amendment credit that a later provider status update
-- previously reversed.
