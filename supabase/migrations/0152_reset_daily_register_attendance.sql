-- Allow an authorised staff member to return one site's daily register to its
-- expected state without cancelling bookings or deleting child reports.
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(entry) order by entry.updated_at, entry.id), '[]'::jsonb)
