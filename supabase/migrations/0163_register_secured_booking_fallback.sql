-- Operational attendance follows secured payment state. This fallback keeps
-- paid/guaranteed sessions visible even if an earlier provider event left a
-- booking header or item in a pre-confirmation state.

create index if not exists booking_items_register_active_date_idx
  on public.booking_items (starts_at, booking_id)
  where status in ('reserved', 'confirmed', 'attended');

create index if not exists booking_items_register_active_site_date_idx
...
begin
  ...
  where item.status in ('reserved', 'confirmed', 'attended')
    and (
...
comment on function public.staff_register_for_day(date, text, text) is
  'Date-indexed operational register showing confirmed or independently secured child sessions, with care details and staff-family markers.';
