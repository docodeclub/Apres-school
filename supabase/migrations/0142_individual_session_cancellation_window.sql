...
begin
  ...
  if not v_parent_can_override and exists (
    select 1
    from booking_items selected_booking_item
    left join sessions selected_session
      on selected_session.id = selected_booking_item.session_id
    where selected_booking_item.booking_id = v_booking.id
      and selected_booking_item.id = any(p_booking_item_ids)
      and selected_booking_item.status in ('reserved', 'confirmed', 'waitlist')
      and v_now > selected_booking_item.starts_at
        - make_interval(hours => greatest(0, coalesce(selected_session.cancellation_hours, 24)))
  ) then
    raise exception 'This session is within its cancellation notice period' using errcode = '42501';
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'insideWindow', true,
      'windowPolicy', 'individual_session_notice'
    )
  );
