...
try {
  ...
  for (const width of [1440, 390, 320]) {
    ...
    assert.equal(await declarations.inputValue(), "");
    assert.equal(await declarations.evaluate(el => el.checkValidity()), false);
    await declarations.selectOption("Needs discussion");
  ...
  console.log("PASS: synthetic desktop/mobile/narrow layouts, declarations, keyboard focus, camp anchor and school context; external requests blocked");
