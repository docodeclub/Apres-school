...
[
  ...
  ["parent checkout shows an in-place submission status", bookingLabSource.includes("lab-checkout-action-status") && bookingLabSource.includes('aria-live="polite"')],
  ...
  ["outstanding-balance failures name the amount and direct parents to payment", bookingLabSource.includes("parentBookingFailureStatus") && bookingLabSource.includes("Payment needed before booking") && bookingLabSource.includes("Open Account → Payments & credit and select Pay now") && bookingLabSource.includes("selected sessions have been kept") && bookingLabSource.includes("openLaunchParentPayments") && bookingLabSource.includes("Open Payments &amp; credit")],
  ["outstanding card invoices are paid through genuine individual checkouts", bookingLabSource.includes("Please pay each invoice separately") && bookingLabSource.includes("No payment has been taken") && bookingLabSource.includes("Pay next invoice") && !bookingLabSource.includes('openParentPaymentCheckout("bulk", familyOutstandingBookingInvoices')],
  ...
  ["parent calendar exposes booking management without switching to list", bookingLabSource.includes('className="lab-parent-calendar-actions"') && bookingLabSource.includes(">Change booking</button>") && bookingLabSource.includes('"Cancel this session"')],
  ["parent account separates multi-block days into cancellable sessions", bookingLabSource.includes("bookingBlockRows(draft).flatMap((row) => row.blocks.map") && bookingLabSource.includes("sessionKey: selectedSessionKey")],
  ["individual cancellation retains the other sessions on the same day", bookingLabSource.includes("remainingBlocks = (row.blocks || []).filter") && bookingLabSource.includes("remainingDayBreakdown.some((row) => row.day === day)")],
  ["booking presets exclude dates before today", bookingLabSource.includes("row.enabled && labDayIso(row.day) >= bookingTodayIso")],
  ...
  ["holiday camp cards use Select Dates as their primary action", bookingLabSource.includes('className="lab-camp-book"') && bookingLabSource.includes(">\n            Select Dates\n          </button>") && bookingLabSource.includes('moveLaunchFlowStep("Dates", ".lab-date-picker-panel")')],
