...
const checks = [
  ...
  ["register provides checkout only for an actively present child", /const canCheckOut = \["checked_in", "late_collection", "incident"\]\.includes\(row\.attendanceStatus\)[\s\S]*onClick=\{\(\) => requestCheckout\(row\)\} disabled=\{busy \|\| !canCheckOut\}>Check out<\/button>/],
