...
const checks = [
  ...
  ["register provides checkout only for an actively present child", /const canCheckOut = \["checked_in", "late_collection", "incident"\]\.includes\(row\.attendanceStatus\)[\s\S]*canCheckOut && <button[\s\S]*requestCheckout\(row\)[\s\S]*>Check out<\/button>/],
  ["register provides only check in and absent for expected children", /const isExpected = row\.attendanceStatus === "booked"[\s\S]*!canCheckOut && <button className="register-action-checkin"[\s\S]*isExpected && <button[\s\S]*>Absent<\/button>/],
  ["register provides four explicit attendance counters", /Visible <strong>\{visibleRows\.length\}<\/strong>[\s\S]*Expected[\s\S]*Present[\s\S]*Checked out[\s\S]*Absent/],
  ["register provides confirmation for bulk check in and respects the filtered view", /setBulkCheckInOpen\(true\)[\s\S]*>Check in all<\/button>[\s\S]*Check in all \{expectedCount\} visible[\s\S]*filters for school, session, care type, or search are respected/],
