...
const checks = [
  ...
  [invoice.includes('["Pricing group"') && invoice.includes("Standard price") && invoice.includes("Discount") && invoice.includes("pricingLabel"), "professional parent invoice shows pricing-group savings"],
  [invoice.includes('doc.bold("Après School"') && invoice.includes("APRES SCHOOL LIMITED") && invoice.includes("Care Sessions") && invoice.includes("Payment Complete"), "parent invoice follows the professional finance invoice hierarchy with correct branding"],
