...
const checks = [
  ...
  ["composer disables invalid or unapproved sends", /disabled=\{!replyAddressValid \|\| !replyReviewed/, content.platform],
  ["corrected recipient is validated server-side", /requestedRecipientEmail[\s\S]*This enquiry does not have a valid reply address/, content.function],
  ["corrected recipient updates the enquiry", /recipientEmail !==[\s\S]*update\(\{ email: recipientEmail \}\)/, content.function],
];
